...
def _load_visualize_module(monkeypatch):
    ...
    fake_datamapplot.selection_handlers = fake_selection_handlers
...
def test_create_topic_map_respects_explicit_topic_tree_true(monkeypatch):
    ...
    viz.create_topic_map(df, word_cloud=False, topic_tree=True)

    assert calls["kwargs"]["enable_topic_tree"] is True
    assert calls["kwargs"]["topic_tree_kwds"]["color_bullets"] is True
...
def test_create_topic_map_uses_native_wordcloud_with_tokens_field(monkeypatch):
    viz, calls = _load_visualize_module(monkeypatch)
    df = _build_df()

    viz.create_topic_map(df, label_column="Name", word_cloud=True)

    handler = calls["kwargs"]["selection_handler"]
    assert "tokens_str" in handler.javascript
