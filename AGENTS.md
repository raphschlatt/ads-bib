...
- `2026-04-09 | Public docs split by user questions while reference/API/output contracts stay explicit | earlier docs had converged in code but still mixed onboarding, runtime choices, schema, API, and repo-internal workflows on too few pages | public docs are now organized around Get Started, Runtime Roads, Search & Query Design, Topic Modeling, Citation Outputs, Python API, and Advanced Integrations, while developer notes keep repo-local runbooks separate | remove remaining public-doc drift and keep internal workflow notes out of the user path`
