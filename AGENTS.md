...
- `2026-04-07 | Bootstrap + doctor as the public onboarding path | packaged CLI users need one repeatable way to scaffold local state and validate provider/runtime prerequisites before long runs | ads-bib bootstrap now owns workspace scaffolding and optional fastText download, while ads-bib doctor resolves the effective config and checks stage-specific blockers | avoid duplicating setup logic across README, get-started, troubleshooting, and CLI help`
