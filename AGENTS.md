...
- `2026-03-13 | Stable GGUF presets pin only runtime-validated architectures | official presets must run on the baseline ADS_env stack without hidden llama.cpp upgrades | local_cpu/local_gpu GGUF defaults stay on model families proven loadable in current llama-cpp-python, while qwen35 remains an explicit upgrade-track option | avoid silent preset breakage from unsupported GGUF architectures`
