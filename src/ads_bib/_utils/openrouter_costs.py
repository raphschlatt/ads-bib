...
def summarize_openrouter_costs(
    ...
    *,
    ...
    if mode_norm == "strict":
        fetch_indices = [i for i, gid in enumerate(generation_ids) if gid]
    elif mode_norm == "hybrid":
        fetch_indices = [i for i, (gid, cost) in enumerate(zip(generation_ids, resolved_costs)) if gid and cost is None]
    else:
        fetch_indices = []
    ...
    if fetch_attempted_calls and api_key:
    ...
    return {
        ...
        "missing_cost_calls": n_calls - priced_calls,
